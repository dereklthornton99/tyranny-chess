...
Hot-seat for two players, an engine opponent at three strengths, and **121 puzzles in four
families** — positions no other chess program can generate. In three of the families the
answer is an execution; in the fourth it is refusing one. One self-contained HTML file, no
build step for the player, no dependencies, no network.
...
121 positions, generated by sweeping real games and testing every position against a
predicate. All four predicates are **depth-free** — exhaustive ply enumeration using only
`legal()`, `apply()` and `inCheck()`, with no search and no engine scoring — which is what
lets one `--seed` reproduce a set exactly.

| Family | Count | Predicate |
|---|---|---|
| **Escape** | 34 | In check, **checkmate under standard rules**, survivable under Tyranny. Every legal move is an execution, and any of them is accepted. |
...
the suite. Over 500 games and 92,699 unique positions on 2026-09-17: escape 0.367 per
thousand unique positions, tactical 0.216, **restraint 1.845**, **multistep 0.291**.
...
because the family rejects every one-mover by construction.
...
tools/gen-puzzles.js    the sweep, the four predicates, and --inline
...
- Puzzle difficulty is better but not solved; see the Puzzles section for the 15 escapes
  that still have exactly one legal move.
